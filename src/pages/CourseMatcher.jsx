...
class CourseMatcher extends Component {
    ...
    state = {
        ...
        userCourses: "",
        timetablesText: [],
        timetablesPlaceholder: <tr>
            <td colSpan="2" className='table-secondary help-text text-muted text-center'><em><small>
    ...
    handleView = (view) => {
        switch (view) {
            ...
            case "sections":
                ...
                break;
            case "meet":
                this.displayMeetBlocks(this.findMeetBlocks());
        ...
        for (let i = 0; i < this.state.courses.length; i++) {
            ...
            for (let a = 0; a < this.state.courses[i].courseList.length; a++) {
                ...
                if (currentCourseFriends.length > 1 && //Check if this is not a duplicate of a previous match record
                    sameCourses.map(e => {
                        return (e.key === currentCourseName)
                    }).every(e => e === false))
        ...
        for (let i = 0; i < this.state.courses.length; i++) {
            ...
            for (let a = 0; a < this.state.courses[i].courseList.length; a++) {
                ...
                if (currentSectionFriends.length > 1 && //Check if this is not a duplicate of a previous match record
                    sameSections.map(e => {
                        return (e.key === currentSectionName)
                    }).every(e => e === false))
    ...
    breakBlocks = (classTimes) => {
        ...
        console.log("class timees", newClassTimes)
    ...
    processQRCode = (result, qrScanner) => {
        ...
        newTimetablesText.push(<tr>
            <td>{this.randomEmoji()} {addedCourse.key}</td>
            <td>
                <button className="btn btn-outline-danger btn-sm deleteBtn" onClick={(e) => this.handleDeleteRow(e)}>X
                </button>
            </td>
        </tr>);
        this.setState({
    ...
    displayErrorMessage = (element, message) => {
        switch (element) {
            case "user":
                this.setState({userErrorMessage: message});
                break;
            case "friends":
                this.setState({friendsErrorMessage: message});
                break;
            default:
                break;
        }
                    ...
                    <div className='col-md-5 p-0 mb-4'>
                        <div className="card">
                            <div className="card-body">
                                ...
                                <div className="m-1">
                                    <p><small className="card-text help-text text-muted">Find your Timetable on your <a
                                        href='https://ssc.adm.ubc.ca/sscportal' rel='noreferrer' target="_blank">SSC</a>,
                                        then click <em>Download your schedule to your calendar software</em>. Upload it
                                        here, then hit Submit.</small></p>
                                </div>
                                <div className="m-1 mb-2">
                                    <div className="row mb-3">
                                        <div className="col-5">
                                            <input type="text" id="user-name" className="form-control"
                                                   placeholder="Your name" onChange={(e) => this.handleChangeName(e)}/>
                                        </div>
                                        <div className="col-7">
                                            <input type="file" id='user-file' accept=".ics" className="form-control"
                                                   onChange={(e) => this.handleUpload(e)}/>
                                        </div>
                                    </div>
                                    <p className='text-danger'>{this.state.userErrorMessage}</p>
                                </div>
                                <p><small className="card-text help-text text-muted">Use the button below to share your
                                    Timetable QR code with your friends.</small></p>
                                <div className="btn-group d-flex px-2 mb-2">
                                    <Button variant={this.state.userSubmitState.color} onClick={this.handleSubmitFile}
                                            disabled={this.state.userSubmitState.disabled}>
                                        {this.state.userSubmitState.text}
                                    </Button>
                                    <Button variant={this.state.shareQRCodeState.color} onClick={this.handleQrCode}
                                            disabled={this.state.shareQRCodeState.disabled}>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                             fill="currentColor" className="bi bi-qr-code mb-1" viewBox="0 0 16 16">
                                    ...
                                    <Modal show={this.state.modalDisplay} onHide={this.handleHideModal}>
                                        ...
                                        <Modal.Footer>
                                            <small className="text-muted">Show this QR code to a friend who can scan it
                                                right now, or take a screenshot and share it with them through social
                                                media and messaging apps!</small>
                                        </Modal.Footer>
                                    </Modal>
                                </div>
                            </div>
                        ...
                        <div className="card">
                            <div className="card-body">
                                <div style={{position: "relative"}}>
                                    ...
                                    <Button variant='primary' style={{position: "absolute", top: 0, right: 0}}
                                            onClick={this.handleScanQRCode}>
                                        <span>+ </span>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                             fill="currentColor" className="bi bi-qr-code-scan mb-1"
                                             viewBox="0 0 16 16">
                                            <path
                                ...
                                <div className="m-1">
                                    <p><small className="card-text help-text text-muted">Scan your friends' Timetable QR
                                        codes, then hit Submit.</small></p>
                                </div>
                                ...
                                <table className="table mb-4">
                                    ...
                                    <tr>
                                        <td colSpan={2}><p className='text-danger'>{this.state.friendsErrorMessage}</p>
                                        </td>
                                    </tr>
                                    </tfoot>
                                </table>
                        ...
                        <Row>
                            <Col md={3} className="mb-3">
                                <Nav variant="pills" className="flex-column">
                                    ...
                                    <Nav.Item>
                                        <Nav.Link eventKey="tab-2" onClick={() => this.handleView("courses")}
                                                  disabled={this.state.pillsState.disabled}>
                                            📚 Courses in common
                                        </Nav.Link>
                                    </Nav.Item>
                                    <Nav.Item>
                                        <Nav.Link eventKey="tab-3" onClick={() => this.handleView("sections")}
                                                  disabled={this.state.pillsState.disabled}>
                                            🧑‍🏫 Sections in common
                                        </Nav.Link>
                                    </Nav.Item>
                                                    ...
                                                    <div className="col">
                                                        <div className="d-inline bg-danger bg-gradient">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                                 ...
                                                                 className="bi bi-square-fill mb-1" viewBox="0 0 16 16"
                                                                 style={{opacity: 0}}>
                                                                <path
